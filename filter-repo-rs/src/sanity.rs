...
pub fn preflight(opts: &Options) -> std::io::Result<()> {
        ...
        .arg("-v"))
    {
        ...
        if !freshly_packed {
            return Err(std::io::Error::new(
                ...
                "sanity: expected freshly packed repo (use --force to bypass)",
            ));
        }
    }
    ...
    if !(remote_trim == "origin" || remote_trim.is_empty()) {
        return Err(std::io::Error::new(
            std::io::ErrorKind::Other,
            "sanity: expected one remote 'origin' or no remotes",
        ));
    }
    ...
    if stash_present {
        return Err(std::io::Error::new(
            std::io::ErrorKind::Other,
            "sanity: stashed changes present",
        ));
    }
    ...
    if !is_bare {
            ...
            .arg("-o"))
        {
            if out.lines().any(|line| {
                ...
                return Err(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    "sanity: untracked files present",
                ));
            }
        }
    }
        ...
        .arg("list"))
    {
        if out.lines().count() > 1 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                "sanity: multiple worktrees found",
            ));
        }
    }
