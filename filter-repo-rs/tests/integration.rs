...
fn analyze_mode_warns_on_commit_thresholds() {
  ...
  assert!(
    report.metrics.oversized_commit_messages.iter().any(|m| m.oid.trim() == long_oid),
    "expected long commit message to be recorded"
  );
  assert!(
    report.warnings.iter().any(|w| w.message.contains(&long_oid)),
    "expected warning mentioning oversized commit message"
  );
  assert!(
    report.warnings.iter().any(|w| w.message.contains("parents")),
...
fn backup_honors_explicit_file_path() {
  ...
  assert!(expected_path.exists(), "expected bundle to exist at {:?}", expected_path);
}
