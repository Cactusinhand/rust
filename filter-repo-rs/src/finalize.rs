...
pub fn flush_lightweight_tag_resets(
  ...
  if buffered_tag_resets.is_empty() { return Ok(()); }
  ...
  for (ref_full, from_line) in items.into_iter() {
    if annotated_tag_refs.contains(&ref_full) { continue; }
    if emitted.contains(&ref_full) { continue; }
    ...
    if let Some(ref mut fi) = fi_in {
      if let Err(e) = fi.write_all(&reset_line) {
        if e.kind() == io::ErrorKind::BrokenPipe { *import_broken = true; } else { return Err(e); }
      }
...
pub fn finalize(
  ...
  if allow_flush_tag_resets {
    ...
    if !buffered.is_empty() {
      flush_lightweight_tag_resets(&mut buffered, &annotated_tag_refs, filt_file, fi_in.as_mut(), &mut import_broken)?;
    }
  }
  if let Some(stdin) = fi_in.take() { drop(stdin); }

  // Handle process termination and propagate errors
  if import_broken { let _ = fe.kill(); }
  ...
  // Write ref-map (debug) and optionally delete old refs
  if !ref_renames.is_empty() {
    ...
    for (old, new_) in &refs {
      f.write_all(&old)?; f.write_all(b" ")?; f.write_all(&new_)?; f.write_all(b"\n")?;
    }
    if !opts.dry_run {
      let mut child = Command::new("git")
        .arg("-C").arg(&opts.target)
        .arg("update-ref").arg("--no-deref").arg("--stdin")
        .stdin(Stdio::piped())
        .spawn()
    ...
    if !status.success() { eprintln!("warning: git update-ref deletions failed: {}", status); }
    }
  }
  ...
  if pairs.is_empty() {
    ...
    if let Ok(fh) = File::open(&filtered) {
      ...
      loop {
        line.clear();
        let n = rdr.read_until(b'\n', &mut line)?; if n == 0 { break; }
        if line.starts_with(b"commit ") { in_commit = true; cur_mark=None; cur_old=None; continue; }
        if !in_commit { continue; }
        ...
        if line.starts_with(b"data ") {
          ...
          let n: usize = std::str::from_utf8(size_bytes).ok().map(|s| s.trim()).and_then(|s| s.parse().ok()).unwrap_or(0);
          let mut buf = vec![0u8; n]; rdr.read_exact(&mut buf)?; continue;
        }
        if line == b"\n" {
          if let (Some(m), Some(old)) = (cur_mark.take(), cur_old.take()) { pairs.push((m, old)); }
          in_commit = false; continue;
        }
      }
    }
  }
  ...
  // Always create commit-map (even if empty) for user tooling parity
  {
    ...
    if !pairs.is_empty() {
      ...
      if let Ok(m) = File::open(&marks_path) {
        let mut rdr = BufReader::new(m);
        ...
        for (mark, old) in pairs {
          if let Some(newid) = mark_to_id.get(&mark) {
            f.write_all(&old)?; f.write_all(b" ")?; f.write_all(newid)?; f.write_all(b"\n")?;
          }
        }
      }
    }
  ...
  if !opts.dry_run && opts.reset {
    ...
    if opts.quiet { reset.arg("--quiet"); }
    reset.arg("--hard");
    let status = reset.status()?;
    if !status.success() { eprintln!("warning: 'git reset --hard' failed: {}", status); }
  }

  // Optional post-import cleanup
  if !opts.dry_run { match opts.cleanup {
    crate::opts::CleanupMode::None => {}
    crate::opts::CleanupMode::Standard | crate::opts::CleanupMode::Aggressive => {
      let _ = Command::new("git")
        .arg("-C").arg(&opts.target)
        .arg("reflog").arg("expire").arg("--expire=now").arg("--all")
        .status();
      let mut gc = Command::new("git");
      gc.arg("-C").arg(&opts.target).arg("gc").arg("--prune=now").arg("--quiet");
      if matches!(opts.cleanup, crate::opts::CleanupMode::Aggressive) { gc.arg("--aggressive"); }
      let _ = gc.status();
    }
  } }
  ...
  if opts.write_report {
    ...
    if let Some(r) = report {
      ...
      if opts.max_blob_size.is_some() {
        ...
        if let Ok(fh) = File::open(&filtered) {
          ...
          while rdr.read_until(b'\n', &mut line).unwrap_or(0) > 0 {
            if line.starts_with(b"D ") {
              ...
              if let Some(last) = p.last() { if *last == b'\r' { p.pop(); } }
        ...
        if size_samples.len() < 20 {
          ...
          if let Ok(fh) = File::open(&original) {
            ...
            let mut oversize_marks: std::collections::HashSet<u32> = std::collections::HashSet::new();
            // Pass 1: collect oversize marks
            loop {
              line.clear();
              if rdr.read_until(b'\n', &mut line).unwrap_or(0) == 0 { break; }
              if line == b"blob\n" { in_blob = true; last_mark = None; continue; }
              if in_blob && line.starts_with(b"mark :") {
                let mut num: u32 = 0; let mut seen=false;
                for &b in line[b"mark :".len()..].iter() { if b>=b'0'&&b<=b'9' { seen=true; num=num.saturating_mul(10).saturating_add((b-b'0') as u32);} else {break;} }
                if seen { last_mark = Some(num); }
                continue;
              }
              if in_blob && line.starts_with(b"data ") {
                let size_bytes = &line[b"data ".len()..];
                let n = std::str::from_utf8(size_bytes).ok().map(|s| s.trim()).and_then(|s| s.parse::<usize>().ok()).unwrap_or(0);
                // consume payload
                let mut buf = vec![0u8; n]; let _ = rdr.read_exact(&mut buf);
                if let (Some(max), Some(m)) = (opts.max_blob_size, last_mark) { if n > max { oversize_marks.insert(m); } }
                in_blob = false; last_mark = None; continue;
              }
              if in_blob && line == b"\n" { in_blob = false; last_mark = None; continue; }
            }
            ...
            loop {
              line2.clear();
              if rdr2.read_until(b'\n', &mut line2).unwrap_or(0) == 0 { break; }
              if line2.starts_with(b"M ") {
                // parse id and path
                let bytes = &line2;
                let mut i = 2; while i < bytes.len() && bytes[i] != b' ' { i+=1; } if i < bytes.len() { i+=1; }
                let id_start = i; while i < bytes.len() && bytes[i] != b' ' { i+=1; } let id_end = i;
                let path_start = if i < bytes.len() { i+1 } else { bytes.len() };
                let id = &bytes[id_start..id_end];
                if id.first().copied() == Some(b':') {
                  let mut num: u32 = 0; let mut seen=false; let mut j=1; while j<id.len(){let b=id[j]; if b>=b'0'&&b<=b'9'{seen=true; num=num.saturating_mul(10).saturating_add((b-b'0')as u32);} else{break;} j+=1;}
      ...
      if !size_samples.is_empty() { writeln!(f, "\nSample paths (size):")?; for p in size_samples { f.write_all(&p)?; f.write_all(b"\n")?; } }
      if !r.samples_sha.is_empty() { writeln!(f, "\nSample paths (sha):")?; for p in r.samples_sha { f.write_all(&p)?; f.write_all(b"\n")?; } }
      if !r.samples_modified.is_empty() { writeln!(f, "\nSample paths (modified):")?; for p in r.samples_modified { f.write_all(&p)?; f.write_all(b"\n")?; } }
    } else {
  ...
  let head_ref = Command::new("git")
      .arg("-C").arg(&opts.target)
      .arg("symbolic-ref").arg("-q").arg("HEAD")
  ...
  if !opts.dry_run {
    if head_ref.status.success() {
      ...
      let ok = Command::new("git")
        .arg("-C").arg(&opts.target)
        .arg("show-ref").arg("--verify").arg(&head)
        .stdout(Stdio::null()).stderr(Stdio::null())
        .status()?.success();
      if !ok {
        ...
        if let Some((ref old, ref new_)) = opts.branch_rename {
          if let Some(tail) = head.strip_prefix("refs/heads/") {
            ...
            if tail_b.starts_with(&old[..]) {
              let mut new_full = Vec::with_capacity("refs/heads/".len() + new_.len() + (tail_b.len().saturating_sub(old.len())));
              ...
              let exists = Command::new("git")
                .arg("-C").arg(&opts.target)
                .arg("show-ref").arg("--verify").arg(&new_str)
                .stdout(Stdio::null()).stderr(Stdio::null())
                .status()?.success();
              if exists { updated_head = Some(new_str); }
            }
          }
        }
        // Choose a suitable branch: updated branch, or first existing branch
        let fallback = updated_head.or_else(|| updated_branch_refs.iter().next().map(|b| String::from_utf8_lossy(b).to_string()))
          .or_else(|| {
            let out = Command::new("git")
              .arg("-C").arg(&opts.target)
              .arg("for-each-ref").arg("--count=1").arg("--format=%(refname)")
              .arg("refs/heads")
              .output().ok()?;
        ...
        if let Some(refstr) = fallback.filter(|s| !s.is_empty()) {
          let status = Command::new("git")
              .arg("-C").arg(&opts.target)
              .arg("symbolic-ref").arg("HEAD").arg(&refstr)
              .status()?;
          if !status.success() { eprintln!("warning: failed to update HEAD to {}: {}", refstr, status); }
        }
      }
    } else {
  ...
  if !opts.quiet { eprintln!("New history written (prototype Rust pipeline). Debug files in {:?}", debug_dir); }
