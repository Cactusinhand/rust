    ...
    for &b in bytes {
        match b {
            ...
            0x00..=0x1F | 0x7F..=0xFF => {
                ...
                let o2 = ((b >> 3) & 0x7) + b'0';
                let o3 = (b & 0x7) + b'0';
                out.push(b'\\'); out.push(o1); out.push(o2); out.push(o3);
            }
            _ => out.push(b),
        }
    }
...
pub fn needs_c_style_quote(bytes: &[u8]) -> bool {
    ...
    for &b in bytes {
        if b <= 0x20 || b >= 0x7F || b == b'"' || b == b'\\' { return true; }
    }
    false
}
...
pub fn glob_match_bytes(pat: &[u8], text: &[u8]) -> bool {
    fn match_from(p: &[u8], t: &[u8]) -> bool {
        ...
        // Handle '?'
        if p[0] == b'?' {
            if t.is_empty() || t[0] == b'/' { return false; }
